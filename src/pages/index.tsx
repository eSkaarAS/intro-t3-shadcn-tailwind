...
export default function Dashboard() {
  ...
  const { mutateAsync: createTodo } = api.todo.create.useMutation({
    onSuccess: () => void utils.todo.findAll.invalidate(),
  });
  const { data: todos } = api.todo.findAll.useQuery();
    ...
    <div className="grid h-screen w-full pl-[56px]">
      ...
      <div className="flex flex-col">
        <header className="sticky top-0 z-10 flex h-[57px] items-center gap-1 border-b bg-background px-4">
          <h1 className="text-xl font-semibold">Todos: {todos?.length ?? 0}</h1>
          <Drawer>
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            const todoText = (e.currentTarget.message as { value: string })
              .value;
            if (!todoText) return;
            void createTodo({
